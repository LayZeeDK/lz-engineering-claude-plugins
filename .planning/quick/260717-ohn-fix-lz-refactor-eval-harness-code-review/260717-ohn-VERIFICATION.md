...
|---|-------|--------|----------|
| 1 | Full offline battery (10 tools/check-*.mjs + selfcheck-code-review + check-evals) exits 0 at HEAD | VERIFIED | Ran all 10 checkers + selfcheck-code-review + check-evals myself at HEAD (dfe56f8): every one exit 0. grade-reference --selfcheck also exit 0. |
| 2 | Each of the 13 fixes is its own atomic bisect-safe commit; touched checker(s) GREEN per commit | VERIFIED | 13 commits 0131a18..dfe56f8 in exact plan order, one logical fix each (correct file scope per `git show --stat`). Spot-checked 7 checker-gated intermediates in-place (FIX1 catalog/gof/extra/functional, FIX2 gof/extra, FIX3 kerievsky, FIX4 smells, FIX5 crossrefs, FIX6 grade-reference --selfcheck, Task11 selfcheck) -- all exit 0. Remaining 6 are isolated additive changes (node --check + HEAD green). |
...
| 8 | Edits ASCII-only; maintainer commit identity is the public gmail; no AI-attribution trailers | VERIFIED | Diff 71f9ee1..HEAD added lines: zero non-ASCII bytes; zero email-shaped tokens introduced (allowlist-inversion clean). All 13 commits author+committer = public gmail. Commit-message scan for co-authored-by/generated-with/claude/anthropic: none. |
...
|-----------|--------|-------------------------------|----------|
| FIX1 / T1 catalog Example-scoped fence + shared sectionBody | 0131a18 | YES | check-catalog.mjs:276-280 scopes fence to `sectionBody(leaf.text,"Example")`; lib/section-body.mjs new + 4 importers |
| FIX5 / T2 exact `^##\s+Example\s*$` in gof + extra | aa86fab | YES | check-gof.mjs:257, check-extra-patterns.mjs:210 both use exact regex |
| FIX7 / T3 kerievsky README-row regex broadened | 4d746b5 | YES | check-kerievsky.mjs:227 `/\]\((?:\.\/)?([a-z0-9][a-z0-9-]*)\.md(?:#[^)\s]+)?\)/g` |
| FIX8 / T4 per-smell recognize-by co-occurrence | 71c8b1d | YES | check-smells.mjs:346-363 per-smell row-scope loop, `rowsMissingCue` -> report(false); "links all 24" assertion retained |
| FIX13 / T5 crossrefs header + github-slug comment accuracy | 70469c8 | YES | check-crossrefs header now states bare `](#fragment)` NOT scanned; github-slug.mjs comment states collapse != github-slugger parity (comment-only, code unchanged) |
| FIX3 / T6 grade-reference flags expected===0 | c5db22a | YES | grade-reference.mjs:419-420 (see Truth 5) |
| FIX6 / T7 eval-status enumerates dirs | a72b9c2 | YES | eval-status.mjs:42-53 (see Truth 6) |
| FIX12 / T8 extract-samples indented fences | d2cc2fe | YES | extract-samples.mjs fence regex `^\s{0,3}\`\`\`(.*)$`; WR-01 unterminated guard intact; offline tsc harness exit 0 (259 modules clean, 0 skipped) |
| FIX9 / T9 prep-ws exit_code gate | fe0fa51 | YES | prep-ws.mjs guard `if(meta.exit_code!==0 \|\| (meta.changed_files\|\|[]).length===0) continue;` |
| FIX10 / T10 tabulate excludes non-zero-exit | faee2b3 | YES | tabulate-mechanical.mjs: `clean=rs.filter(r=>r.exit===0)`, `nClean`; drove/union/liftMean/cLift/Pass@k over clean, liftMean guarded (`nClean ? ... : '0.0'`), cost/turns/tools over all `rs` |
| FIX4 / T11 selfcheck skips crux-3 on ENOENT | 7d473ef | YES | selfcheck-code-review.mjs:230-234 `if(err.code==='ENOENT'){ SKIP; return; }`; non-ENOENT still `fail(...)` |
| FIX2 / T12 run-e2e apply refuses to orphan commits | 735727f | YES | run-e2e.mjs:512-518 `const ahead=(git(cwd,['rev-list',\`${APPLY_BASE}..HEAD\`]).stdout\|\|'').trim(); if(ahead) throw` -- placed after PROTECTED_BRANCHES (502) and before reset --hard (524)/clean -fd (525); buildSyntheticBase untouched |
| FIX11 / T13 run-e2e synthetic branch process-unique | dfe56f8 | YES | run-e2e.mjs:403 `branchName=\`review-${promptEntry.target}-${process.pid}-${Date.now()}\``; still `review-`-prefixed (glob-compatible); no residual literal `review-<target>` branch refs |
...
No gaps. 13 atomic bisect-safe commits exist in plan order (FIX2 apply-guard 735727f and FIX11
synthetic-branch dfe56f8 are separate run-e2e.mjs commits). Every cited behavior change is present at
