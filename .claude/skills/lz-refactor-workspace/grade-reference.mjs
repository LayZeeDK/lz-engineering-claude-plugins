...
// ---- clause-scoped negation awareness ----
// Negation cues that, when present in a token's own clause BEFORE it, flip it from a claim to a
// denial. Clause boundaries are punctuation OR a contrastive conjunction (a negation before "but"
// does not carry into the clause after it), so "not a Fowler smell but a Kerievsky one" affirms
// Kerievsky, and "It is not a Fowler smell, it is Kerievsky's" affirms Kerievsky.
// ponytail: punctuation + contrastive-conjunction heuristic, not a parser. Applied identically to
// both arms, so an occasional mis-read cancels in the delta. Upgrade to a real clause splitter only
// if a graded phrasing proves systematically mis-scored between arms.
...
// Token appears at least once un-negated in its clause.
function occursAffirmed(resp, token) {
  const re = nameReAll(token);
  let m;

  while ((m = re.exec(resp)) !== null) {
    if (!NEG.test(clauseBefore(resp, m.index))) {
      return true;
    }
...
// ---- aggregate: per-question per-arm correct-rate + primary A/B delta + Pass@k ----
// perArmQ shape: { <arm>: { <qid>: [bool, ...] } }.
function aggregate(perArmQ, targets) {
  const questions = {};
  ...
  for (const arm of fs.readdirSync(modeDir)) {
    ...
    for (const qid of fs.readdirSync(armDir)) {
      ...
      for (const run of fs.readdirSync(qDir)) {
        const ans = path.join(qDir, run, "answer.md");
...
function runAggregate() {
  ...
  const perArmQ = walkResults(targets);
  ...
  const agg = aggregate(perArmQ, targets);
...
function selfcheck() {
  ...
  const agg = aggregate(perArmQ, dtargets);
