...
- [x] **EVL-03**: An APPLY-based, 3-arm, multi-dimensional RED eval -- short human prompts drive the next failing test in >= 1 real OSS TypeScript repo across no_skill / with_skill / invoke_skill arms; the produced test file/diff is graded on a correctness GATE (tsc --strict differential clean AND genuinely red for the right reason) plus lift dims (wall-clock, tokens, tools, output quality, book/source authenticity via oracle-reviewer, idioms, house style, TDD RED practices), with Pass@k/Pass^k + >= 1 unbiased reviewer and a substance-only headline. (Phase 21 BUILT + deterministically verified the instrument/harness/grader/selfcheck. The empirical metered run RAN 2026-07-27/28 on fresh approval -- 36 runs, $29.04, across GRC/RXF/RXL/SRVC -- and is written up in `.claude/skills/lz-red-workspace/e2e-red-gilded-rose/EVAL-RESULTS.md` with a substance-only headline, Pass@k/Pass^k per cell, mechanical + blind-judge lift dims, and the mandatory unbiased reviewer. RECONCILED 2026-08-02: this line previously still read "Pending until run" months after the round completed. All 36 captures were re-graded on 2026-08-02 under the CURRENT grader -- which had since gained a `blunt_red` class and a widened `verdictPass()` -- with ZERO verdict changes, so the published numbers are grader-regime-invariant and stand as written. LAST DIMENSION CLOSED 2026-08-02: book/source authenticity via oracle-reviewer was the one lift dim named in ROADMAP SC3 that had never been run -- it was recorded in EVAL-RESULTS.md as a deliberate skip on the RESEARCH A5 low-power prediction, but THIS line previously listed it among the graded dims, which overstated coverage. The owner commissioned the measurement rather than accept the argument, and it is now RUN: all 36 captured produced tests graded blind against the owned RED sources by four independent oracle-reviewer agents (one per cell), zero metered spend. Result is NULL and the prediction held -- authentic-of-12 is 7 `no_skill` / 6 `with_skill` / 5 `invoke_skill` (Pass@1 0.58 / 0.50 / 0.42), a two-specimen spread that is a TIE at this n and must not be read as a reverse effect, with all four graders independently reporting the axis has little discriminating power on this corpus. One corroboration of value: the single inauthentic verdict in 36 is the same capture the blind judge independently flagged as its only dim-1 miss. A corpus defect (only `+` lines extracted, reproducing the documented first-corpus bug) was caught mid-pass; those numbers were discarded and the corpus rebuilt via `git show` + `git apply`, 36/36 applied. EVL-03 is now complete with every SC3 lift dimension measured rather than argued. COMPLETE.)
  ...
  - **EVL-03.3** (BUILD, maps SC3): The produced-test correctness GATE (D-06) classifies a run into one of the 9 classes {genuinely_red / blunt_red / false_green / drove_to_green / compile_error / collection_error / no_tests / wrong_reason / unattributable} and passes exactly two of them, genuinely_red and blunt_red (tsc --strict differential-clean AND an assertion failure on current code, in a test the diff added). Proven by grade-red --selfcheck over fixture test-pairs (zero spend, fixtures pristine). (WORDING CORRECTED 2026-08-02: this line described a 7-class gate passing ONLY genuinely_red while enumerating just 6 classes. The shipped `VERDICTS` array in grade-red.mjs has 9 entries and `verdictPass()` admits blunt_red as well. The widen is deliberate, not drift -- it was invariance-proven by re-grading all 36 captures under the current grader with zero verdict changes, and no capture in the round is classified blunt_red -- but the requirement text had not been refreshed to match the instrument.)
...
| EVL-03 | Phase 21 | Complete (run 2026-07-27/28, 36 runs, $29.04; re-graded 2026-08-02, 0 verdict changes) |
