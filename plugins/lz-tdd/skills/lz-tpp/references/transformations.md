...
the earlier 12-item list). Entries are verbatim-faithful with arrows normalized to ASCII
`->`; descriptions are given where the source gives them (FibTPP post):
...
- TPP post -- Robert C. Martin, "The Transformation Priority Premise" (Clean Coder Blog):
  ...
  Source of: the transformations-vs-refactorings definition, the premise, the priority
  rationale, the mantra, the author's hedges (except "language specific"), and the amended
...
- FibTPP post -- Robert C. Martin, "Fib. The T-P Premise." (Clean Coder Blog):
  ...
  Source of: the canonical revised 14-item list and the "language specific" caveat.
