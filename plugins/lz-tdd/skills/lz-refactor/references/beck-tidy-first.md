...
- Explaining variables -- introduce a well-named variable for a sub-expression:
  [Extract Variable](./fowler-catalog/extract-variable.md).
- Extract helper -- pull a chunk into a named unit: [Extract Function](./fowler-catalog/extract-function.md),
...
- Inline an over-eager helper -- undo an extraction that no longer earns its name:
...
- Move declaration and initialization together -- bring a variable next to its first use for reading
  order: [Slide Statements](./fowler-catalog/slide-statements.md).
- Guard clauses -- flatten nested conditionals into early exits:
