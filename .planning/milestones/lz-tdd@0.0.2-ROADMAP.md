# Milestone Archive: lz-tdd@0.0.2 -- lz-refactor Skill (Fowler + Kerievsky)

**Status:** SHIPPED 2026-07-17
**Phases:** 6-14 (11 phase directories, incl. inserted 8.1 GoF and 8.2 Functional)
**Total Plans:** 55
**Requirements:** 36/36 satisfied (see `lz-tdd@0.0.2-REQUIREMENTS.md`)
**Audit:** passed (see `lz-tdd@0.0.2-MILESTONE-AUDIT.md`)

> Archived snapshot of `ROADMAP.md` as it stood at the lz-tdd@0.0.2 ship. For current
> project status see `.planning/ROADMAP.md`.
...
- [x] 08.2-02-PLAN.md -- 6 N:1 dissolves-from idiom leaves (first-class-function, thunk-and-lazy-value, function-composition, discriminated-union-and-fold, module-namespace, factory-function) + per-pattern residual anchors (wave 2)
- [x] 08.2-03-PLAN.md -- 7 1:1 dissolves-from idiom leaves (reducer-and-store, observer-callbacks, generator, memoization-and-interning, immutable-snapshot, structural-clone, branded-type) (wave 2)
- [x] 08.2-04-PLAN.md -- 6 alternative-to native FP leaves (option-and-either, functor-and-monad, lens-and-optics, currying-and-partial-application, transducers, normalized-store) (wave 2)
- [x] 08.2-05-PLAN.md -- N:1 pattern->idiom map README finalize + D-14 Replace-Pipeline-with-Loop reverse note in the Fowler leaf (wave 3)
- [x] 08.2-06-PLAN.md -- 55 Functional alternative: cross-links + SKILL.md router pointer + full battery/typecheck/validate/skill-reviewer gate (wave 4)
...
- [x] 13-01-PLAN.md -- Behavior-oracle baselines (nx differential ~15-fail reference; kata pristine golden master); non-metered

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 13-02-PLAN.md -- Metered apply runs behind the D-09 gate: no_skill single-target backfill (p1/p2/gr1) + sweep pair (p8 no_skill; gr4 both arms) per amended D-02

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 13-03-PLAN.md -- Book-authenticity grading via the oracle agent (every applied refactoring; pass/partial/fail; DST-04)
- [x] 13-04-PLAN.md -- Correctness grading (name/layer vs targets.json + independent behavior oracle: nx differential / kata jest --ci)
...
- [x] 14-03-PLAN.md -- grading scaffolding: deterministic mechanical tabulator (tokens/tools/spawn/lift/Pass@k) + head-to-head rollup template (D-04/D-07)
