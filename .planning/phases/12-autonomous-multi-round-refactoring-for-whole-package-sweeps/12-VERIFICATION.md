...
audit_notes:
  ...
  - finding: "SUMMARY-vs-codebase divergence. 12-02-SUMMARY.md and 12-03-SUMMARY.md describe a 'broadened description' (the whole-package-sweep clause from commit 98eedd5) as part of the shipped skill. It was REVERTED post-phase by 138acf4 ('revert 12-02 sweep bulk') on measurement grounds (null value-delta). The shipped auto-trigger is carried by the 8acd2b8 intent-based description (which REMAINS in HEAD) -- independently validated at 18/18 and re-confirmed post-revert via kata gr4 3/3. The sweep-DRIVE cluster remains (leaner rebuild). Documentation narrative diverges from HEAD but no SC fails."
    severity: warning
  - finding: "RESOLVED 2026-07-14 (was: evidential nuance on nx sweep trigger). Originally the nx whole-package sweep auto-trigger was measured PRE-revert (quick 260712-i5y p8 3/3; quick 260712-n5o fleet p9-p12 8/8) and only the KATA sweep (gr4 3/3) was directly re-confirmed on HEAD post-revert, leaving the nx sweep strongly inferred. Closed by a gated, user-approved metered re-run on HEAD (quick 260714-vmy): p8 with_skill apply k=3 -> 3/3 auto-triggered (used_refactor=true, skills_invoked=[lz-tdd:lz-refactor] each run; deterministic from meta.json), matching pre-revert p8 3/3 and post-revert kata gr4 3/3. No regression from 138acf4. The nx post-revert auto-trigger is now DIRECTLY MEASURED on HEAD, not inferred."
    severity: resolved
---
...
|---|-----------|--------|----------|
| 1 | Natural whole-package sweep prompt auto-triggers in BOTH suites, measured before/after, WITHOUT regressing coach-recommend (18/18) or specificity | VERIFIED | E2E-FINDINGS.md Resolution: nx coach 1/12 -> 12/12, kata 0/6 -> 6/6, combined **1/18 -> 18/18** (8acd2b8 desc; commits ee95cd4 [pruned]/e7fcc8c). Isolated trigger-eval recall 92% -> 100%, specificity held **100% (11/11 quiet)**. Package-scope sweep: nx p8 **3/3** + kata gr4 **3/3** (quick 260712-i5y); nx fleet p9-p12 **8/8** (quick 260712-n5o); kata gr4 **3/3 re-confirmed post-revert** (GR-RESULTS.md, 2026-07-14); nx p8 **3/3 re-confirmed post-revert on HEAD** (quick 260714-vmy, 2026-07-14). Current shipped description = 1245 chars, ASCII-clean, `claude plugin validate` PASS. |
...
|----------|----------|--------|---------|
| `plugins/lz-tdd/skills/lz-refactor/SKILL.md` (sweep-drive cluster) | "Whole-package / directory sweeps" loop-to-fixpoint section | VERIFIED | Present lines 101-116 (leaner rebuild from 138acf4). All guards wired: fixpoint, tests-green-each-round, blast-radius pause, revert-on-red, checkpoint. 158 lines (<500), ASCII-clean. |
| `plugins/lz-tdd/skills/lz-refactor/SKILL.md` (description) | Intent-based description firing on sweep-shaped prompts | VERIFIED | Shipped description = 8acd2b8 intent/readability rewrite (1245 chars), validated 18/18; nx sweep p8 3/3 directly re-measured on HEAD post-revert (quick 260714-vmy, 2026-07-14). NOTE: the additional 12-02 "sweep a whole package" clause (98eedd5) was REVERTED (138acf4); it is NOT in HEAD, contra 12-02/12-03 SUMMARY narrative. |
| `plugins/lz-tdd/skills/lz-refactor/SKILL.md` (step-4 net-cost warrant) | Forced APPLY/DECLINE verdict before code | VERIFIED | Present lines 63-76 (877aef3 "one countable net-cost test"); L1 accepted (068e46e, both ancestors of HEAD). gr4 3/3 emitted countable DECLINE. |
...
|------|----|----|--------|---------|
| natural sweep prompt | lz-refactor auto-invocation | SKILL.md `description` (8acd2b8) | WIRED | 18/18 e2e + gr4 3/3 + nx p8 3/3 (both post-revert, HEAD); description in HEAD, validate PASS |
...
| HEAD ancestry | `git merge-base --is-ancestor` 138acf4 / 068e46e | both ancestors of HEAD (64be50b) | PASS |
...
None. All four Success Criteria are met by measured e2e evidence already captured (deterministic trigger/drive rates from `meta.json`); there is no UI, visual, real-time, or external-service surface. The one previously-optional item -- re-confirming the nx whole-package sweep auto-trigger on HEAD post-revert -- was COMPLETED 2026-07-14 via a gated, user-approved metered re-run (quick 260714-vmy: p8 with_skill apply k=3 -> 3/3 auto-triggered on HEAD); it is now directly measured, not inferred.
...
2. **[Doc-accuracy warning]** The 12-02/12-03 SUMMARY "broadened description" was reverted post-phase (138acf4); the shipped trigger is carried by the 8acd2b8 description that remains and is validated. SUMMARY narrative diverges from HEAD.
3. **[Resolved 2026-07-14]** nx sweep auto-trigger now directly measured on HEAD post-revert (quick 260714-vmy: p8 with_skill apply k=3 -> 3/3), closing the prior "inferred, not re-measured" nuance. No regression from 138acf4.
