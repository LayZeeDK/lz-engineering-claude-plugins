#!/usr/bin/env node
// FWL-04 compile harness. Walks the shipped Fowler catalog leaves, extracts every fenced
// `ts` block into its OWN module under samples/ (one module per fence so before/after
// snippets that reuse the same symbol names never collide), then runs tsc --strict --noEmit.
