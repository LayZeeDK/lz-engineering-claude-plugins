...
|-----------|----------|-----------|-------------|------------|--------|
| T-02-01-01 | Information Disclosure | Retained transcript in public `.planning/` | mitigate | Work-email allowlist scan (public gmail only; check never spells the work-email literal). Verified: transcript email-free; `consensus.dk` scan zero matches | closed |
...
| T-02-02-03 | Information Disclosure | Work-email in a public committed file | mitigate | Work-email allowlist scan over shipped file + phase dir (never spells the literal). Verified: no email after filtering public gmail; `consensus.dk` zero matches | closed |
