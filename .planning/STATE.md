...
| 2026-07-15 | groupby-detection-variants | complete | Deeper research-informed 2nd attempt at the SAME T4 blind spot (--full --research --auto). RESEARCH found the prior edit was dead weight (left the GATE cue smells.md:64 -- scanned FIRST by coach step 2 -- byte-unchanged; mirror not byte-enforced). Variant A+B (--auto-locked): broadened the gate cue in both mirrored files to name reduce/accumulator group-by + added O(n^2) acc.find cost to the loops leaf; reviewer PASS, gates green. Metered cr-rlu invoke_skill k=3 (indices 7-9, $1.33): **T4 STILL 0/3 (zero groupImports mentions), T3 3/3**. Fails pre-locked bar (T4>=2/3) -> **variant REVERTED** (checkout HEAD~1). DEFINITIVE: model recall/judgment ceiling, not a catalog-detection gap -- catalog edits cannot close it. Two experiments ($2.94 total) confirm. Stop attacking via the skill. See quick/20260715-groupby-detection-variants |
