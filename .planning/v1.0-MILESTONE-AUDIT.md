...
tech_debt:
  - phase: cross-cutting
    items:
      ...
      - "Version-numbering mismatch: GSD tracks this milestone as v1.0 (STATE.md milestone: v1.0) while the product ships semver 0.0.1. Reconcile the label at complete-milestone."
...
# Milestone v1.0 (product 0.0.1) -- Audit Report
...
> **Version note:** GSD's internal milestone identifier for this cycle is `v1.0` (per `.planning/STATE.md`), which is why this artifact is filed as `v1.0-MILESTONE-AUDIT.md`. The **product release version** it ships is semver **0.0.1** (`plugin.json`, ROADMAP header). The two numbering systems are independent and both correct; the mismatch is recorded as non-blocking tech debt to reconcile at `complete-milestone`.
...
- GSD milestone id `v1.0` vs product semver `0.0.1` mismatch -- reconcile the label at complete-milestone.
...
Milestone **v1.0 (product 0.0.1)** achieved its definition of done: 24/24 requirements satisfied,
