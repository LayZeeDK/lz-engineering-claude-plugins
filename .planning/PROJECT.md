...
| `lz-red` source access model: owned sources live git-ignored in `.oracle/` clean-room (own-words only, DST-04) -- books (RCM *Clean Code*; Metz + Owen *99 Bottles JS Ed*, for test naming) plus a transcribed-talk + written-content library under `.oracle/videos/` and `.oracle/written-content/` (Metz *The Magic Tricks of Testing* for the message matrix, *The Design of Tests*, and other Metz talks; Gary Bernhardt *Boundaries*; Kent Beck *Test Desiderata* + essays; Ian Cooper); remaining unowned refs = no-oracle high-confidence core; DHH hard-banned | Copyright hygiene: never commit verbatim copyrighted transcripts/prose; ship own-words synthesis | Good -- 0.0.3 Phase 17 uses these owned sources (message-matrix source corrected 99 Bottles -> Magic Tricks 2026-07-19) |
