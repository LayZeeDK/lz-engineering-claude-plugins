...
| [Rename Variable](rename-variable.md) | none | a variable's name does not convey what it holds, and a clearer name would capture what you now understand. |
...
| [Substitute Algorithm](substitute-algorithm.md) | none | you have found a clearer way to do what a block of code already does, and want to swap the whole approach rather than tweak the old one. |
...
| [Move Field](move-field.md) | none | a field is used more by another record than the one that currently holds it, or fields that change together live apart. |
| [Move Statements into Function](move-statements-into-function.md) | none | the same statements always run alongside a call to a function, so they belong inside it. |
| [Move Statements to Callers](move-statements-to-callers.md) | none | statements once common to every caller of a function now need to vary between them. |
| [Replace Inline Code with Function Call](replace-inline-code-with-function-call.md) | none | inline code does the same thing as a function that already exists. |
...
| [Split Loop](split-loop.md) | none | a single loop does two different things, so you cannot change one without understanding the other. |
| [Replace Loop with Pipeline](replace-loop-with-pipeline.md) | none | a loop walks a collection through steps (filtering, mapping, accumulating) that a collection pipeline would state more directly. |
| [Remove Dead Code](remove-dead-code.md) | none | code is no longer executed or referenced (an unused variable, function, parameter, or branch). |
...
| [Rename Field](rename-field.md) | none | a record or class field's name no longer says what it holds. |
| [Replace Derived Variable with Query](replace-derived-variable-with-query.md) | none | a variable stores a value that could be computed from other data, kept in sync by update code. |
| [Change Reference to Value](change-reference-to-value.md) | none | a small nested object is shared and mutated in place, but would be simpler treated as an immutable value. |
| [Change Value to Reference](change-value-to-reference.md) | none | many copies of the same conceptual entity exist as separate value objects, but they should be one shared object. |

## Ch.10: simplifying conditional logic

| Refactoring | 1st-ed aliases | Use when |
|---|---|---|
| [Decompose Conditional](decompose-conditional.md) | none | a conditional's test or branches are complex enough that their intent is hard to read. |
| [Consolidate Conditional Expression](consolidate-conditional-expression.md) | none | several separate conditional checks all lead to the same result or action. |
| [Replace Nested Conditional with Guard Clauses](replace-nested-conditional-with-guard-clauses.md) | none | nested conditionals make the normal path of execution hard to pick out. |
| [Replace Conditional with Polymorphism](replace-conditional-with-polymorphism.md) | none | a conditional chooses behavior by type or kind, and that variation could be expressed with subclasses. |
...
| [Introduce Assertion](introduce-assertion.md) | none | a section of code works only if some condition holds, but that assumption is left implicit. |

## Ch.11: refactoring APIs

| Refactoring | 1st-ed aliases | Use when |
|---|---|---|
| [Separate Query from Modifier](separate-query-from-modifier.md) | none | a function both returns a value and changes observable state, so callers cannot ask the question without causing the effect. |
...
| [Preserve Whole Object](preserve-whole-object.md) | none | a caller pulls several values out of one object just to pass them into a function. |
...
| [Replace Query with Parameter](replace-query-with-parameter.md) | none | a function reaches out to global state or a wide reference that would be better passed in. |
| [Remove Setting Method](remove-setting-method.md) | none | a field should be set once at creation and never changed afterward. |
...
| [Replace Function with Command](replace-function-with-command.md) | Replace Method with Method Object | a function is complex enough (lots of local state, or wanting to be broken up) that turning it into an object would help. |
| [Replace Command with Function](replace-command-with-function.md) | none | a command object does little more than run one function, so the class is just overhead. |
| [Return Modified Value [web-only]](return-modified-value.md) | none | a function updates a value by mutating it in place, and the code would read more clearly if it returned the new value instead. |

## Ch.12: dealing with inheritance

| Refactoring | 1st-ed aliases | Use when |
|---|---|---|
| [Pull Up Method](pull-up-method.md) | none | two or more subclasses carry methods that do the same thing, so the behavior belongs on the shared parent. |
| [Pull Up Field](pull-up-field.md) | none | sibling subclasses each declare a field that holds the same thing. |
| [Pull Up Constructor Body](pull-up-constructor-body.md) | none | subclass constructors begin with the same setup steps. |
| [Push Down Method](push-down-method.md) | none | a method on the superclass is relevant to only one subclass (or a few), not all of them. |
| [Push Down Field](push-down-field.md) | none | a field on the superclass is used by only one subclass (or a few), not all of them. |
...
| [Extract Superclass](extract-superclass.md) | none | two classes share fields and behavior that mark them as variants of one idea. |
| [Collapse Hierarchy](collapse-hierarchy.md) | none | a superclass and its subclass have grown so alike that the separation no longer earns its keep. |
| [Replace Subclass with Delegate](replace-subclass-with-delegate.md) | none | subclasses vary one aspect of an object, but inheritance is too rigid: the axis changes at runtime, or you need to vary more than one thing at once. |
